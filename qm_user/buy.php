...
$moneycards = '';
$membertypes = '';
$dsql->SetQuery("SELECT * FROM #@__moneycard_type ORDER BY tid ASC");
$dsql->Execute();
while($row = $dsql->GetObject())
{
    $row->money = sprintf("%01.2f", $row->money);
    $moneycards .= "<tr align='center'>
    <td id='radio'><input type='radio' id='pid' name='pid' value='{$row->tid}'><input type='hidden' id='jiage' name='jiage' value='{$row->money}'></td>
    <td>{$row->pname}</td>
    <td>{$row->num}个</td>
    <td>{$row->money}元</td>
    </tr>
...
while($row = $dsql->GetObject())
{
    ...
    <td id='radio'><input type='radio' id='pid' name='pid' value='{$row->aid}'></td>
    <td>{$row->pname}</td>
