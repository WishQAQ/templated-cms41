...
function CheckRank($rank=0, $money=0, $needinfo=TRUE)
{
    ...
    if(!$cfg_ml->IsLogin())
    {
        header("Location:{$cfg_cmsurl}/i/?type=login&gourl=".urlencode(GetCurUrl()));
        exit();
    }
    else
    {
        if($cfg_mb_reginfo == 'Y' && $needinfo)
        {
            ...
            {
                ShowMsg("尚未完成详细资料，请完善...","{$cfg_memberurl}/index_do.php?fmdo=user&dopost=regnew&step=2",0,8);
                exit;
            }
        }
...
/** 会员头像的配置 **/
$row_face = $dsql->GetOne("SELECT * FROM `#@__member` WHERE mid='{$cfg_ml->M_ID}'");
if($row_face['face'] == '')
{
	if($row_face['matt'] == '10'){
		$cfg_ml->fields['face'] = $GLOBALS['cfg_cmsurl'].'/images/admin_pic.png';
	}else{
		$cfg_ml->fields['face'] = $GLOBALS['cfg_cmsurl'].'/images/noface.gif';
	}
}
