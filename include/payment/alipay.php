...
class Alipay
{
	...
	var $dsql;
    var $mid;
    var $return_url = "/payback/carbuyaction.php?dopost=return";
    /**
     * 构造函数
     *
     * @access  public
    ...
    */
    function GetCode($order, $payment)
    {
        ...
        $parameter = array(
          ...
          /* 业务参数 */
          'subject'           => "站窝窝模板网增值服务，支付订单号:".$order['out_trade_no'],
    ...
    */
    function respond()
    {
		...
		{
            $row = $this->dsql->GetOne("SELECT * FROM #@__member_operation WHERE mid='".$this->mid."' AND buyid = '{$order_sn}'");
            //获取订单信息，检查订单的有效性
            if($row['sta'] == 2)
			{
        ...
        {
            if($ordertype=="goods"){ 
                if($this->success_db($order_sn))  return $msg = "支付成功!<br> <a href='/'>返回主页</a> <a href='/i/'>会员中心</a>";
                else  return $msg = "支付失败！<br> <a href='/'>返回主页</a> <a href='/i/'>会员中心</a>";
            } else if ( $ordertype=="member" ) {
                $oldinf = $this->success_mem($order_sn,$pname,$product,$pid);
                return $msg = "<font color='red'>".$oldinf."</font><br> <a href='/'>返回主页</a> <a href='/i/'>会员中心</a>";
            }
        } else {
            $this->log_result ("verify_failed");
            return $msg = "支付失败！<br> <a href='/'>返回主页</a> <a href='/i/'>会员中心</a>";
        }
    }
       ...
       if($product=="card")
        {
            ...
            $nrow = $this->dsql->GetOne("SELECT * FROM #@__member_operation WHERE buyid = '$order_sn'");
            $dnum = $nrow['money'];
            $sql1 = "UPDATE `#@__member` SET `money`=money+'{$nrow['money']}' WHERE `mid`='".$this->mid."'";
            $oldinf ="已经充值了".$nrow['money']."金币到您的帐号！";
            //更新交易状态为已关闭
    ...
    {
        global $cfg_cmspath;
        $fp = fopen(dirname(__FILE__)."/../../qm_data/payment/log.txt","a");
        flock($fp, LOCK_EX) ;
