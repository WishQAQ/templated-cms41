...
class Archives
{
    ...
    function ParAddTable()
    {
		...
		$aid = $this->Fields['id'];
		$demo_url = $this->Fields['demo_url'];
		$arctitle = $this->Fields['title'];
		if($demo_url == '')		{
			$this->Fields['msg'] = "<div style='margin-top:200px;font-size:14px;font-weight:normal;'>暂无演示，2秒后将为你跳转到模板详情页</div>";
			$this->Fields['msgtitle'] = "暂无演示，2秒后将为你跳转到模板详情页";
			$this->Fields['gourl-d'] = "<meta http-equiv='refresh' content='2;URL=".$cfg_cmsurl."/moban/".$aid.".html' />";
		}else{
			$this->Fields['msg'] = "";
			$this->Fields['msgtitle'] = $arctitle."在线演示";
		}
        // 处理投票选项
    ...
    function MakeHtml($isremote=0)
    {
        ...
        $filename = GetFileNewName(
            ...
            "demo/d-u".$this->TypeLink->TypeInfos['namerule']
        );
    ...
    function LoadTemplet()
    {
        if($this->TempSource=='')
        {
            ...
            $this->dtp->LoadTemplate('../demo/plus/demo_url.htm');
    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
                ...
                $this->PreNext['pre'] = "<a class'pre-post' href='$mlink' rel='prev'><i class='fa fa-arrow-circle-left'></i>上一篇：{$preRow['title']}</a> ";
                $this->PreNext['preimg'] = "<a href='$mlink'><img src=\"{$preRow['litpic']}\" alt=\"{$preRow['title']}\"/></a> "; 
            }
            else
            {
                $this->PreNext['pre'] = "<a class'pre-post' href=\"javascript:alert('没有上一篇了');\" rel='prev'><i class='fa fa-arrow-circle-left'></i>上一篇：没有了</a>";
            ...
            if(is_array($nextRow))
            {
                ...
                $this->PreNext['next'] = "<a class='next-post' href='$mlink' rel='next'>下一篇：{$nextRow['title']}<i class='fa fa-arrow-circle-right'></i></a> ";
                $this->PreNext['nextimg'] = "<a href='$mlink'><img src=\"{$nextRow['litpic']}\" alt=\"{$nextRow['title']}\"/></a> ";
            }
            else
            {
                $this->PreNext['next'] = "<a class='next-post' href=\"javascript:alert('没有下一篇了');\" rel='next'>下一篇：没有了<i class='fa fa-arrow-circle-right'></i></a>";
