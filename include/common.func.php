...
function ShowMsg($msg, $gourl, $onlymsg=0, $limittime=0)
{
    ...
    if($gourl == "-1"){
		...
		$tzgourl = "<span>$gourl_time</span> 秒后跳转至";
		$gourl_title = "上一页";
		include(DEDETEMPLATE.'/0226/msg.htm');
		exit();
    }else if($gourl == 'javascript:window.close();'){
         $tzgourl = "<span>$gourl_time</span> 秒后跳转至";
		 $gourl_title = "将关闭窗口";
		 include(DEDETEMPLATE.'/0226/msg.htm');
		 exit();
    }else if($gourl == $GLOBALS['cfg_cmsurl']."/i/?type=login"){
         $tzgourl = "<span>$gourl_time</span> 秒后跳转至";
		 $gourl_title = "用户登录";
		 include(DEDETEMPLATE.'/0226/msg.htm');
		 exit();
		...
		$tzgourl = "<span>$gourl_time</span> 秒后跳转至";
		global $cfg_basehost;
		if($gourl == $GLOBALS['cfg_basehost'] || $gourl == '/' || $gourl == $GLOBALS['cfg_cmsurl']){
			$gourl_title = "网站首页";
		}else if($gourl == $GLOBALS['cfg_cmsurl']."/i/?type=do&fmdo=sendMail"){
			$gourl_title = "发送邮件";
		}else if($gourl == '/i/' || $gourl == $GLOBALS['cfg_cmsurl'].'/i/'){
			$gourl_title = "用户中心";
		}else{
			$gourl_title = $gourl;
		}
		include(DEDETEMPLATE.'/0226/msg.htm');
		exit();
	}else if($gourl != '-1' && $gourl != 'javascript:window.close();'){
