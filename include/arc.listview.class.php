...
class ListView
{
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
        ...
        }else if($orderby=="dows") {
            $ordersql = "  ORDER BY arc.dows $orderWay";
        }else if($orderby=="likes") {
            $ordersql = "  ORDER BY arc.likes $orderWay";
        }else {
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        {
            return "<span class=\"jumppage total-page\">该类下没有任何数据</span>\r\n";
        }
        $purl = $this->GetCurUrl();
        //$maininfo = "<span class=\"jumppage total-page\">共{$totalpage}页</span>\r\n";
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a class=\"prev page-numbers\" href='".str_replace("{page}",$prepagenum,$tnamerule)."'><i class=\"fa fa-chevron-left\"></i></a>\r\n";
			$prepage.="<span class=\"page-numbers dots\">...</span>\r\n";
        }
       
        //下一页,未页的链接
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
			$nextpage.="<span class=\"page-numbers dots\">...</span>\r\n";
            $nextpage.="<a class=\"next page-numbers\" href='".str_replace("{page}",$nextpagenum,$tnamerule)."'><i class=\"fa fa-chevron-right\"></i></a>\r\n";
        }
       	
		if($this->TotalResult >= 20)
		{
			$tz = "<span class='jumppage go-page' id='go-page' ><input id='page' class='jump-to' value='1' name='page' type='text'><a href=\"javascript:go_jump2('list_".$this->Fields['id']."_'+$('#page').val()+'.html',$('#page').val(),200)\" title='立即跳转' id='jump-btn' class='jump-btn'>Go</a></span>\r\n";
		}
        ...
        for($j;$j<=$total_list;$j++)
        {
            if($j==$this->PageNo)
            {
                //$listdd.= "<span class=\"page-numbers current\">$j</span>\r\n";
            }
            else
            {
                $listdd.="<a class=\"page-numbers\" href='".str_replace("{page}",$j,$tnamerule)."'>".$j."</a>\r\n";
            }
        }
        $plist = '';
		...
		if(preg_match('/tz/i', $listitem)) $plist .= $tz;
     ...
     * @return    string
     */
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno,tz")
    {
        global $cfg_rewrite;
        $prepage = $nextpage = '';
        $prepagenum = $this->PageNo-1;
       ...
       // $maininfo = "<span class=\"jumppage total-page\">共{$totalpage}页</span>\r\n";
		...
		if($this->TotalResult >= 20)
		{
			$tz = "<span class='jumppage go-page' id='go-page' ><input id='page' class='jump-to' value='1' name='page' type='text'><a href=\"javascript:go_jump2('".$purl."".$pageaddurl."PageNo='+$('#page').val(),$('#page').val(),200)\" title='立即跳转' id='jump-btn' class='jump-btn'>Go</a></span>\r\n";
		}
		
        //获得上一页和下一页的链接
        if($this->PageNo != 1)
        {
			$prepage.="<a class=\"prev page-numbers\" href='".$purl."PageNo=$prepagenum".$pageaddurl."'><i class=\"fa fa-chevron-left\"></i></a>\r\n";
			$prepage.="<span class=\"page-numbers dots\">...</span>\r\n";
        }
        ...
        {
			$nextpage.="<span class=\"page-numbers dots\">...</span>\r\n";
            $nextpage.="<a class=\"next page-numbers\" href='".$purl."PageNo=$nextpagenum".$pageaddurl."'><i class=\"fa fa-chevron-right\"></i></a>\r\n";
        }
        ...
        for($j;$j<=$total_list;$j++)
        {
            ...
            {
                $listdd.="<a class=\"page-numbers\" href='".$purl."PageNo=$j".$pageaddurl."'>".$j."</a>\r\n";
            }
        }
