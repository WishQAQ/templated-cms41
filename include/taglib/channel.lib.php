...
function lib_channel(&$ctag,&$refObj)
{
    ...
    else if($type=='son')
    {
		...
		$c = $dsql->GetOne("select count(*) as dd from `#@__archives` where typeid='$typeid'");
    }
    else if($type=='self')
    {
        if($reid==0) return '';
    ...
    for($i=0;$i < $line;$i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row=$dsql->GetArray())
            {
                ...
                //处理同级栏目中，当前栏目的样式
                if( ( $row['id']== $refObj->TypeLink->TypeInfos['id'] || ($topid==$row['id'] && $type=='son') ) && $currentstyle!='' )
                {
					...
					$linkOkstr = str_replace("~count~",$c['dd'],$linkOkstr);
                    $likeType .= $linkOkstr;
                }else{
