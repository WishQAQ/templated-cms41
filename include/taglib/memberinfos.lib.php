...
function lib_memberinfos(&$ctag,&$refObj)
{
    ...
    {
        if($row['matt']==10){
			$row['face']= $GLOBALS['cfg_basehost'].$GLOBALS['cfg_cmsurl'].'/images/admin_pic.png';
		}
        $row['spaceurl'] = $GLOBALS['cfg_basehost'].'/i/?uid='.$row['userid'];
        if(empty($row['face'])) {
            $row['face']=($row['sex']=='女')?  $GLOBALS['cfg_cmsurl'].'/images/noface.gif' : $GLOBALS['cfg_cmsurl'].'/images/noface.gif';
        }
