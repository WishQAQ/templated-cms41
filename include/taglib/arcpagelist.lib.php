...
function lib_arcpagelist(&$ctag, &$refObj)
{
    ...
    if(is_array($row))
    {
      ...
      $totalnum = count($ids);
      $pagestr = '<div id="page_'.$tagid.'" class="pagination loop-pagination">';
      ...
      $pagestr = '<div id="page_'.$tagid.'" class="pagination loop-pagination">';
      $pagestr .= '<span class="jumppage total-page">没有检索到对应分页</span>';
      $pagestr .= '</div>';
        return $pagestr;
    }
}
...
function multipage($allItemTotal, $currPageNum, $pageSize, $tagid='')
{
    ...
    for ($i=($currPageNum-4); $i<($currPageNum+9); $i++) {
        ...
        if ($i == $currPageNum) $listNums.= "<span class='page-numbers current'>".$i."</span>";
        else $listNums.= " <a href='javascript:multi(". $i .",\"{$tagid}\")' class='page-numbers'>". $i ."</a> ";
    }
